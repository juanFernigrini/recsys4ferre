...
async def health():
    ...
    user_feature_sparse_matrix = csr_matrix(user_features.values)
    item_feature_sparse_matrix = csr_matrix(item_features.values)

    A = csr_matrix(np.concatenate((user_item_sparse_matrix.todense(), user_feature_sparse_matrix.todense()), axis=1))
    B = csr_matrix(np.concatenate((item_feature_sparse_matrix.todense(), np.eye(len(item_features))), axis=1))


    # SVD
    U, sigma, Vt = svds(A.dot(B.T), k=50) # k is the number of singular values to compute
...
async def consulta(customer_id):
    ...
    user_row = user_item_matrix.iloc[customer_id_int, :]
    user_feature_row = user_features.iloc[customer_id_int, :]
    ...
    user_predicted_purchase_counts = np.dot(U[customer_id_int,:], np.dot(np.diag(sigma), Vt[:, :len(item_features.columns)]))
    item_predicted_purchase_counts = np.dot(U[customer_id_int,:], np.dot(np.diag(sigma), Vt[:, len(item_features.columns):]))

    #Ordenamos obtener los artículos más recomendados para ese usuario e imprimimos:
    user_recommendations = pd.DataFrame({'CodArticu': user_item_matrix.columns, 'predicted_purchase_count': item_predicted_purchase_counts})
