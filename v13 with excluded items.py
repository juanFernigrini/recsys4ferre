...
async def consulta(customer_id, exclude_items: List[int] = []):
    ...
    remaining_recommendations = user_recommendations.head(10 - len(novelty_diversity_recommendations))

    # Combine the novelty and diversity recommendations with the remaining recommendations
    final_recommendations = pd.concat([novelty_diversity_recommendations, remaining_recommendations])

    # Add a column to distinguish between recommendation types
    ...
    final_recommendations.loc[remaining_recommendations.index, 'recommendation_type'] = "remaining_recommendations"

    # Extract the item IDs and their corresponding types
