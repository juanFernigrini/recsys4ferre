...
async def preparar_modelo():
    global matrix, U, sigma, Vt, data, item_similarity, data
    ...
    # Grid search for the best number of components
    param_grid = {'n_components': [1, 100, 300, 500]}
    svd = TruncatedSVD()
    model = GridSearchCV(svd, param_grid, scoring='neg_mean_squared_error', n_jobs=-1)
    model.fit(sparse_matrix)
    best_n_components = model.best_params_['n_components']
    
    print (best_n_components)
    
    U, sigma, Vt = entrenar_modelo(sparse_matrix, best_n_components)
...
def entrenar_modelo(sparse_matrix, n_components):
