...
async def health(k: int = 50):
    ...
    for k in range(1, 100):
        U, sigma, Vt = svds(sparse_matrix, k=k) # k is the number of singular values to compute
        all_user_predicted_purchase_counts = np.dot(np.dot(U, np.diag(sigma)), Vt) + np.mean(matrix, axis=0).values.reshape(1, -1)
        rmse = sqrt(mean_squared_error(matrix.values, all_user_predicted_purchase_counts))
        rmse_list.append(rmse)

    # Get best k
    best_k = rmse_list.index(min(rmse_list)) + 1
    print(f"Best k value is: {best_k}")

    # Train model with best k
    U, sigma, Vt = svds(sparse_matrix, k=best_k)
    all_user_predicted_purchase_counts = np.dot(np.dot(U, np.diag(sigma)), Vt) + np.mean(matrix, axis=0).values.reshape(1, -1)
...
async def consulta():
    ...
    # SVD
    U, sigma, Vt = svds(sparse_matrix, k=best_k) # k is the number of singular values to compute
